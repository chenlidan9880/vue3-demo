<template>
  <div class="booking-list">
    <header class="header">
      ...
      <p class="subtitle">查看和管理你的所有预约</p>
    </header>

    <main class="content">
      <div v-if="loading" class="state">加载中...</div>
      <div v-else-if="error" class="state error">{{ error }}</div>
      <div v-else-if="list.length === 0" class="state">你还没有任何预约，去浏览房源吧</div>
      ...
      <div v-else class="booking-grid">
        <div v-for="booking in list" :key="booking.id" class="booking-card">
          ...
          <div class="card-footer">
            <button class="btn btn-primary" @click="goToDetail(booking.id)">
              查看详情
            </button>
            <button 
...
export default {
  ...
  methods: {
    ...
    applyRefund(bookingId) {
      this.$router.push(`/bookings/${bookingId}/refund`)
    },
