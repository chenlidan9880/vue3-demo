        ...
        >
          ...
          <div class="handle-result" v-if="complaint.status === 2 || complaint.status === 3">
            ...
            <div v-if="complaint.status === 2">
              <p class="result-content success">{{ complaint.handleResult }}</p>
              <div class="result-tip success">
                <span class="tip-icon">✓</span>
                <span>投诉已成立，平台已对房东/房源采取相应措施</span>
              </div>
            </div>
          ...
          <div class="complaint-actions">
            <button class="btn btn-primary" @click="goToComplaint(complaint.bookingId)">
              再次投诉
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    goToComplaint(bookingId) {
      this.$router.push(`/bookings/${bookingId}/complaint`)
    },
    goBack() {
